...
public class BadgeButton extends JButton {
    ...
    private Color badgeColor = ComponentConfig.BADGE_BUTTON_COLOR;

    public BadgeButton() {
        setUI(new BadgeButtonUI());
        setOpaque(false);
        setForeground(ComponentConfig.BADGE_BUTTON_FOREGROUND);
        setBorder(ComponentConfig.BADGE_BUTTON_BORDER);
    ...
    // convert the icon to the BufferedImage object
    ...
    private class BadgeButtonUI extends BasicButtonUI {
        ...
        @Override
        protected void paintText(Graphics graphics, JComponent jc, Rectangle rctngl, String string) {

        }
        
        @Override
        protected void paintIcon(Graphics graphics, JComponent jc, Rectangle iconRect) {
            if (image != null && !getText().equals("")) {
                // paint manually if text isn't empty
                ...
                g2.dispose();
            } else { // if text is empty, use the default paint method
                super.paintIcon(graphics, jc, iconRect);
            }
        }
